...
const SUPABASE_CONFIG = {
  url: 'https://ffzcvirrckmwqnwiqxpi.supabase.co',
  anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZmemN2aXJyY2ttd3Fud2lxeHBpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg2MDU0MzIsImV4cCI6MjEwNDE4MTQzMn0.LFM3WoQvbALcvQ_ayXA_0MnuaYxkp-gB2l1SK7mbrLs'
};
...
async function deleteProductionEntry(id) {
  ...
  if (isSupabaseActive && supabaseClient) {
    try {
      await supabaseClient.from('production_entries').delete().eq('id', id);
    } catch (err) {
...
async function clearAllProductionEntries() {
  ...
  if (isSupabaseActive && supabaseClient) {
    try {
      await supabaseClient.from('production_entries').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    } catch (err) {
...
function startLiveClock() {
  const clockEl = dom.liveClock || document.getElementById('live-clock');
  const shiftEl = dom.shiftName || document.getElementById('shift-name');
  const update = () => {
    const now = new Date();
